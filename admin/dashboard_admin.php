...
=======
            <div class="flex items-center justify-between">
            <div>
            <h3 class="text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wide">Total Buku</h3>
            <?php
                $query = "SELECT COUNT(*) AS total_buku FROM buku";
                $result = mysqli_query($config, $query);
                $data = mysqli_fetch_assoc($result);
                $total_buku = $data['total_buku'];
            ?>
            <p class="text-3xl font-bold text-blue-600"><?php echo $total_buku; ?></p>
            <p class="text-xs text-gray-500 mt-1">Tersedia di sistem</p>
            </div>
            <div class="bg-blue-secondary/20 p-3 rounded-full">
            <img src="../assets/img/logobook.png" alt="Book Icon" class="w-8 h-8">
...
=======
            <div class="flex items-center justify-between">
            <div>
            <h3 class="text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wide">Total Anggota</h3>
            <?php
                $query = "SELECT COUNT(*) AS total_anggota FROM users WHERE role != 'admin'";
                $result = mysqli_query($config, $query);
                $data = mysqli_fetch_assoc($result);
                $total_anggota = $data['total_anggota'];
            ?>
            <p class="text-3xl font-bold text-teal-600"><?php echo $total_anggota; ?></p>
            <p class="text-xs text-gray-500 mt-1">Member aktif</p>
            </div>
            <div class="bg-teal-primary/20 p-3 rounded-full">
            <img src="../assets/img/groupusers.png" alt="Members Icon" class="w-8 h-8">
...
=======
            <div class="flex items-center justify-between">
            <div>
            <h3 class="text-xs font-semibold text-gray-600 mb-2 uppercase tracking-wide">Peminjaman Aktif</h3>
            <?php 
                $query = "SELECT COUNT(*) AS total_peminjaman FROM transaksi WHERE status = 'dipinjam'";
                $result = mysqli_query($config, $query);
                $data = mysqli_fetch_assoc($result);
                $total_peminjaman = $data['total_peminjaman'];
            ?>
            <p class="text-3xl font-bold text-cyan-600"><?php echo $total_peminjaman; ?></p>
            <p class="text-xs text-gray-500 mt-1">Sedang dipinjam</p>
            </div>
            <div class="bg-cyan-accent/20 p-3 rounded-full">
            <img src="../assets/img/pinjam.png" alt="Borrow Icon" class="w-8 h-8">
...
=======
            ...
            <?php 
                ...
                $total_dikembalikan = $data['total_dikembalikan'];
            ?>
            <p class="text-3xl font-bold text-emerald-600"><?php echo $total_dikembalikan; ?></p>
            <p class="text-xs text-gray-500 mt-1">Sudah dikembalikan</p>
            </div>
            <div class="bg-emerald-accent/20 p-3 rounded-full">
            <img src="../assets/img/file.png" alt="Return Icon" class="w-8 h-8">
...
=======
            ...
            <div class="space-y-3">
                <?php
                $query = "SELECT u.nama, t.status, t.tanggal_pinjam 
                      FROM transaksi t 
                      JOIN users u ON t.id_user = u.id_user 
                ...
                <div class="flex items-center justify-between p-3 <?= $status_class; ?> rounded-lg">
                <div class="flex items-center gap-3">
...
=======
            ...
            <div class="space-y-3">
                ...
                <span class="text-sm font-medium text-gray-700 group-hover:text-blue-secondary">Kelola Buku</span>
                <svg class="w-4 h-4 ml-auto text-gray-400 group-hover:text-blue-secondary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                </a>
                <a href="daftar_anggota.php" class="flex items-center gap-3 p-3 bg-gradient-to-r from-teal-50 to-teal-100 rounded-lg border-l-4 border-teal-primary hover:shadow-md transition-all group">
                <div class="bg-teal-primary/20 p-2 rounded-lg group-hover:bg-teal-primary/30">
                    <img src="../assets/img/groupusers.png" alt="Members" class="w-5 h-5">
                </div>
                <span class="text-sm font-medium text-gray-700 group-hover:text-teal-primary">Kelola Anggota</span>
                        <svg class="w-4 h-4 ml-auto text-gray-400 group-hover:text-teal-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    </a>
>>>>>>> 689875abed4c8ff882dfb89705d62e0fa103442f
...
=======
                        <span class="text-sm font-medium text-gray-700 group-hover:text-cyan-accent">Kelola Peminjaman</span>
                        <svg class="w-4 h-4 ml-auto text-gray-400 group-hover:text-cyan-accent" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg>
                    </a>
                </div>
            </div>
        </div>          
